...
export function findIntersection(linePositions: THREE.BufferAttribute | THREE.InterleavedBufferAttribute, intersectingObject: THREE.Mesh) {
  ...
  const direction = new THREE.Vector3().subVectors(line3.end, line3.start).normalize();
  ...
  if (intersects.length > 0) {
    ...
    return { intersectionSphere, intersectionPoint };
  } else {
